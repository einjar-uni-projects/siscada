...
public class ReceiveAutomaton1 extends Thread{
	...
	public void run(){
		...
		if(_masterContext.get_contextoAut1().getDispositivosInternos(_configutarion.getPosicionAsociada(NombreMaquinas.FIN_1)) &&
			 ...
			 * y tengo un blister con posicion libre, es decir el contador es menor que 4
			 */
		...
		if(_masterContext.get_contextoAut1().getPastelesRestantes()<_configutarion.getUmbralPasteles()){
			/*
			 * si el contexto del aut 1 me dice q quedan pocos pasteles envio el mensaje a SCADA de pocos pasteles
			 */
			MessageInterface mensajeSend=new DefaultMessage();
			mensajeSend.getParametros().add(NombreMaquinas.DISPENSADORA.getName());
			mensajeSend.getParametros().add(_masterContext.get_contextoAut1().getPastelesRestantes()+"");
			mensajeSend.setIdentificador(OntologiaMSG.AVISARUNFALLO);
			_postmaster.sendMessageSCADA(mensajeSend);
		}
		if(_masterContext.get_contextoAut1().getCapacidadCaramelo()<_configutarion.getUmbralCaramelos()){
			/*
			 * si el contexto del aut 1 me dice q queda poco caramelo envio el mensaje a SCADA de poco caramelo
			 */
			MessageInterface mensajeSend=new DefaultMessage();
			mensajeSend.getParametros().add(NombreMaquinas.CARAMELO.getName());
			mensajeSend.getParametros().add(_masterContext.get_contextoAut1().getCapacidadCaramelo()+"");
			mensajeSend.setIdentificador(OntologiaMSG.AVISARUNFALLO);
			_postmaster.sendMessageSCADA(mensajeSend);
		}
		if(_masterContext.get_contextoAut1().getCapacidadChocolate()<_configutarion.getUmbralChocolate()){
			/*
			 * si el contexto del aut 1 me dice q queda poco chocolate envio el mensaje a SCADA de poco chocolate
			 */
			MessageInterface mensajeSend=new DefaultMessage();
			mensajeSend.getParametros().add(NombreMaquinas.CHOCOLATE.getName());
			mensajeSend.getParametros().add(_masterContext.get_contextoAut1().getCapacidadChocolate()+"");
			mensajeSend.setIdentificador(OntologiaMSG.AVISARUNFALLO);
			_postmaster.sendMessageSCADA(mensajeSend);
		}
	}
