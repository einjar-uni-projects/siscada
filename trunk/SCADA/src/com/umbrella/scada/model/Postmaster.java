...
public class Postmaster extends Thread {
	...
	private Postmaster() throws PropertyException {
		...
		_clientMailBox = new ClientMailBox(pfmodel.getMasterAutIP(), pfmodel
				.getMasterAutPort(), ServerMailBox._reciveSCADAName,
				ServerMailBox._sendSCADAName, true);
				...
				if (msg != null) {
					...
					case ROBOT_SET_CONTENT:
						...
						params.setParam(ape, ape.getEnclosedClass(), msg
								.getParameters().get(0));
						af
								.executeAction(ActionKey.UPDATE_ROBOT_CONTENT,
										params);
						break;
					case ACTUALIZARCONFIGURACION:
						...
						params.setParam(ape, ape.getEnclosedClass(), msg
								.getObject());
						af
								.executeAction(ActionKey.UPDATE_CONFIGURATION,
										params);
						break;
